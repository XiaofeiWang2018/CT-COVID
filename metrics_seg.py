...
if __name__ == '__main__':
    ...
    import os
    seg_data1 = np.zeros(shape=(512, 512, 301), dtype=np.float64)
    ...
    if (sysstr == "Linux"):
        gt_nii = nb.load('../dataset/covid/segment_label/3.nii')
    gt_nii = np.uint8(gt_nii.dataobj)
    gt_nii = np.transpose(gt_nii, (1,0,2)) ###
    gt_nii = gt_nii[::-1, ...]  ### [512,512,200]
    gt_ni=gt_nii[:,:,0:243]
    gt_ni[gt_ni > 1] = 1
    gt = np.expand_dims(gt_ni, axis=0)  # (1,512,512,200)
    gt = np.concatenate(((1 - gt), gt), axis=0)# (2,512,512,200)
    gt = np.expand_dims(gt, axis=0)  # (1,2,512,512,200)
    gt_lesion_3d=np.transpose(gt[0][1],(2,0,1))
    gt_lesion=gt[0][1].reshape(-1)# (512*512*200)

    # predict = nb.load('D:\covid\segment_label/4.nii')
    # predict = np.uint8(predict.dataobj)
    # predict = np.transpose(predict, (1, 0, 2))  ###
    # predict = predict[::-1, ...]  ### [512,512,200]
    # predict[predict > 1] = 1
    # # predict=np.float32(predict)# (1,512,512,200)
    # predict = np.expand_dims(predict, axis=0)  # (1,512,512,200)
    # predict = np.concatenate(((1 - predict), predict), axis=0)  # (2,512,512,200)
    # predict = np.expand_dims(predict, axis=0)  # (1,2,512,512,200)
    # predict_lesion=predict[0][1].reshape(-1)# (512*512*200)

    predict=gt.copy()
    predict[:,:,:,:,0:100]=0
    predict_3d=np.transpose(predict[0][1],(2,0,1))
    predict_lesion = predict[0][1].reshape(-1)  # (512*512*200)

    #dice
    begin_time = time()
    dice_lesion=dice(test=predict_lesion,reference=gt_lesion)
    end_time = time()
    ...
    begin_time = time()
    surface_distances = compute_surface_distances(gt_lesion_3d, predict_3d, spacing_mm=(1.5, 1.2, 1.2))
    ...
    # sensi
    begin_time = time()
    sensi_lesion = sensitivity(test=predict_lesion, reference=gt_lesion)
    end_time = time()
    run_time = end_time - begin_time
    print('sensi: ', run_time)

    # spec
    begin_time = time()
    spec_lesion = specificity(test=predict_lesion, reference=gt_lesion)
    end_time = time()
    run_time = end_time - begin_time
    print('spec: ', run_time)
    ...
    # asd
    begin_time = time()
    asd_lesion = avg_surface_distance_symmetric(test=predict_lesion, reference=gt_lesion)
    end_time = time()
    run_time = end_time - begin_time
    print('asd: ', run_time)
    ...
    # AUC
    begin_time = time()
    AUC_lesion = roc_auc_score(y_true= gt_lesion, y_score=predict_lesion)
    end_time = time()
    run_time = end_time - begin_time
    print('AUC: ', run_time)
    ...
    # AP
    begin_time = time()
    ap_lesion = average_precision_score(y_true= gt_lesion, y_score=predict_lesion)
    end_time = time()
    run_time = end_time - begin_time
    print('ap: ', run_time)
    ...
    # ct=np.load('D:\covid\data-npy/10.npy')
    # ct =np.transpose(ct,(1,2,0))
    # ct=ct[::-1, ...]### [512,512,200]
    # plt.imsave('ct_uint8.jpg',ct[:,:,132],cmap=plt.get_cmap('gray'))
    # plt.imsave('gt_uint8.jpg', gt_nii[:, :, 132], cmap=plt.get_cmap('gray'))
